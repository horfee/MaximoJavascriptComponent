...
String scriptFID = component.getId().replaceAll("-","_");
boolean debugEnabled = control.getWebClientSession().getDebug();
...
if ( designmode ) {
    ...
    %>
    <div style="display: flex; align-items: center;" id="<%=component.getId()%>" <%=componentEvents%>>

    <%
    if ( scriptControl.getScriptType() == ScriptControl.ScriptType.JAVASCRIPT ) {
        %>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" style="width: 32px; height: 32px;">
...
} else if ( scriptControl.getContent() != null ) {
    if ( component.needsRender()  ) {
        String eventType = "render";
            ...
            <script type="text/javascript" id="<%=component.getId()%>" data-id="<%=component.getId()%>">
                _<%=scriptFID%> = async function() {
                    ...
                    if ( scriptControl.getScriptType() == ScriptControl.ScriptType.JAVASCRIPT ) {
                        ...
                        %><%=scriptControl.getContent()%><%
                    } else if (scriptControl.getScriptType() == ScriptControl.ScriptType.CSS) {
                    %>
                        const existingStyle = document.querySelector('style[data-id="<%=component.getId()%>"]');
                        if ( existingStyle ) {
                            existingStyle.remove();
                        }
                        const style = document.createElement('style');
                        style.setAttribute('data-id', '<%=component.getId()%>');
                        style.innerHTML = `<%=scriptControl.getContent()%>`;
                ...
                if ( window.ScriptControl === undefined ) {
                    if ( document.querySelector("script#scriptcontrol") === null ) {          
                        const script = document.createElement("script");
                        script.id = "scriptcontrol";
                        script.src = "<%=servletBase%>/javascript/<%=maximoJSDirectory%>scriptcontrol.js";
                        script.type = "text/javascript";
                        document.head.appendChild(script);
                    }

                    document.querySelector("script#scriptcontrol").addEventListener("load", function() {
                        new ScriptControl("<%=component.getId()%>").apply(_<%=scriptFID%>);
                    });
                } else {
                    notifyScriptControl("<%=component.getId()%>", "<%=eventType%>", _<%=scriptFID%>);
                }
            </script>
        <%
    } else {
        %>
        <component id="<%=component.getId()%>_holder"><![CDATA[
            <script type="text/javascript" id="<%=component.getId()%>" data-id="<%=component.getId()%>">
                notifyScriptControl("<%=component.getId()%>", "update");
            </script>
        ]]></component>
        <%
    }
